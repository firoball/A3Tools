...
int triangulate_monotone_polygons(int nvert, int nmonpoly, int* op)
{
  ...
  for (i = 0; i < nmonpoly; i++)
	...
	{
	  op[3*op_idx+0] = mchain[p].vnum;
	  op[3*op_idx+1] = mchain[mchain[p].next].vnum;
	  op[3*op_idx+2] = mchain[mchain[p].prev].vnum;
	  op_idx++;
	}
 ...
 */
static int triangulate_single_polygon(int nvert, int posmax, int side, int* op)
{
	register int v;
  ...
  while ((v != endv) || (ri > 1))
    {
	  ...
	  {
/* 01/23/11 patched by firoball
 * cross product was checked for > before
 * in order to support overlapping vertices
 * change it to >=
 */
	  if (CROSS(vert[v].pt, vert[rc[ri - 1]].pt, 
		    vert[rc[ri]].pt) >= 0)
	    {			/* convex corner: cut if off */
	      op[3*op_idx+0] = rc[ri - 1];
	      op[3*op_idx+1] = rc[ri];
	      op[3*op_idx+2] = v;
	      op_idx++;	     
	      ri--;
...
/* 01/23/11 patched by firoball
 * ri may become 0 here which will cause a crash
 * DUDE! it was a nightmare to find out that one
 * I can't explain in words HOW MUCH I hate this source!
 */
	if (ri > 0)
	{
